...
function Page() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const { data: etudiant, error: etuErr } = await supabase
      .from("etudiants_pre_inscrits").select("id").eq("etablissement_id", etabId).eq("email", email.trim().toLowerCase()).maybeSingle();
    if (etuErr || !etudiant) { setError("Vous n'êtes pas pré-inscrit. Contactez votre établissement."); setBusy(false); return; }
