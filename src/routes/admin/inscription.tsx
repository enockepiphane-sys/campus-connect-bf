...
import { UserPlus, Building2, BookOpen, Mail, Calendar, Lock } from "lucide-react";
...
function Page() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // Step 1: Verify against admins_pre_autorises
    ...
    if (adminErr || !admin) {
      setError("Vous n'êtes pas pré-autorisé pour cet établissement.");
      setBusy(false);
      return;
    }

    if (admin.date_naissance && dateNaissance && admin.date_naissance !== dateNaissance) {
      setError("La date de naissance ne correspond pas.");
      setBusy(false);
      return;
    }
    ...
    // Step 3: Link the user_id and set inscrit = true
    if (authData.user) {
      await supabase
        .from("admins_pre_autorises")
        .update({ user_id: authData.user.id, inscrit: true })
        .eq("id", admin.id);
