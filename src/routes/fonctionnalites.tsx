...
import {
  ClipboardList, Megaphone, Calendar, Clock, Users, BookOpen,
  ShieldCheck, UserPlus, LogIn, Building2, FileSpreadsheet,
} from "lucide-react";
  ...
  return (
    <PageShell title="Fonctionnalités">
      <p className="mb-10 max-w-2xl text-lg text-muted-foreground">
        CampusLink met à disposition des universités et étudiants du Burkina Faso une suite d'outils simples et sécurisés pour gérer la vie académique au quotidien.
      </p>
      ...
      <section className="mb-12">
        <div className="card-glass rounded-2xl p-8">
          <div className="flex items-start gap-4">
            <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-primary-soft text-primary"><ShieldCheck className="h-6 w-6" /></div>
            <div>
              <h2 className="mb-2 text-xl font-bold text-foreground">Sécurité & confidentialité</h2>
              <p className="text-sm text-muted-foreground">Chaque étudiant ne voit que ses propres notes. Les annonces, événements et emplois du temps sont limités à son niveau et sa filière. Les administrateurs ne peuvent accéder qu'aux données de leur propre établissement. Les données sensibles ne sont jamais publiques.</p>
            </div>
          </div>
        </div>
      </section>
...
function FeatureCard({ icon, title, desc, color }: { icon: React.ReactNode; title: string; desc: string; color: "primary" | "accent" | "gold" }) {
  const bg = { primary: "bg-primary-soft text-primary", accent: "bg-accent text-accent-foreground", gold: "bg-gold/20 text-gold-foreground" }[color];
  return (
...
function StepBlock({ step, icon, title, desc }: { step: string; icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="flex items-start gap-3">
      <div className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-primary-soft text-primary">{icon}</div>
      <div>
        <h3 className="font-semibold text-foreground"><span className="mr-1 text-primary">{step}.</span> {title}</h3>
