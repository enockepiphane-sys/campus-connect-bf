...
import { UserPlus, Building2, Mail, Calendar, Lock } from "lucide-react";
...
function Page() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const { data: admin, error: adminErr } = await supabase
      ...
      .select("id, etablissement_id, date_naissance")
      .eq("etablissement_id", etabId)
      .eq("email", email.trim().toLowerCase())
      .maybeSingle();
    if (adminErr || !admin) { setError("Vous n'êtes pas pré-autorisé pour cet établissement."); setBusy(false); return; }
    if (admin.date_naissance && dateNaissance && admin.date_naissance !== dateNaissance) { setError("La date de naissance ne correspond pas."); setBusy(false); return; }
    ...
    if (authData.user) {
      await supabase.from("admins_pre_autorises").update({ user_id: authData.user.id, inscrit: true }).eq("id", admin.id);
      await supabase.from("user_roles").insert({ user_id: authData.user.id, role: "admin", etablissement_id: etabId });
    }
